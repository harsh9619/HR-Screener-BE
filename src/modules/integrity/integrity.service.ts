 ...
 */
export function detectPromptInjection(resumeText: string): IntegrityCheckResult {
 ...
 */
export function detectTimelineInconsistency(resumeText: string): IntegrityCheckResult {
  ...
  const dateRangeRegex =
    /(?:([A-Za-z]+|\d{1,2})[\/\s,]+)?(\d{4})\s*[\u2013\u2014-]\s*(?:([A-Za-z]+|\d{1,2})[\/\s,]+)?(\d{4}|Present|Current)/i;
    ...
    for (let j = i + 1; j < periods.length; j++) {
      ...
      if (overlapEnd - overlapStart > 60 * 24 * 60 * 60 * 1000) {
        return {
          ...
          explanation:
            'These employment periods overlap significantly and may require recruiter review. Note: Concurrent employment, consulting, part-time roles, or resume formatting quirks can be legitimate and this is flagged for recruiter verification.',
 ...
 */
export function detectTemplatedInflation(resumeText: string): IntegrityCheckResult {
  const normalized = normalizeResumeText(resumeText);
  ...
  for (const [, matchingLines] of Object.entries(percentMap)) {
    if (matchingLines.length >= 2) {
      return {
        ...
        explanation:
          'Similar quantified achievement language or identical performance metrics appear repeatedly across multiple roles. This requires human review to confirm individual contributions.',
  ...
  const lineCounts: Record<string, string[]> = {};
  for (const line of lines) {
 ...
 */
export function runIntegrityCheckPipeline(resumeText: string): AnalysisPipelineResult {
