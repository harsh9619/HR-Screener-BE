 ...
 */
export function calculateFitScore(resumeText: string, requirements: RoleReqInput[]): FitScoreResult {
  ...
  for (const req of requirements) {
    ...
    if (req.isMustHave) {
      mustHaveCount++;
      if (isMatched) mustHaveMatches++;
    } else {
      optionalCount++;
      if (isMatched) optionalMatches++;
    }
