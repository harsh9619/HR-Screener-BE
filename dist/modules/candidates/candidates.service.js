...
exports.deleteCandidate = exports.compareCandidates = exports.getCandidateDetails = exports.listCandidatesByRole = exports.createCandidate = void 0;
const uuid_1 = require("uuid");
const candidatesQuery = __importStar(require("./candidates.query"));
const integrity_service_1 = require("../integrity/integrity.service");
const scoring_service_1 = require("../scoring/scoring.service");
const createCandidate = async (roleId, input) => {
    ...
    const roleRequirements = rawReqs.map((r) => ({
        requirement: r.requirement,
        isMustHave: Boolean(r.isMustHave),
    }));
    const integrityResult = (0, integrity_service_1.runIntegrityCheckPipeline)(input.resumeText);
    const scoreResult = (0, scoring_service_1.calculateFitScore)(input.resumeText, roleRequirements);
    const candidateId = (0, uuid_1.v4)();
    const now = new Date().toISOString();
    await candidatesQuery.insertCandidate(candidateId, roleId, input.name.trim(), input.email.trim().toLowerCase(), input.resumeText, integrityResult.status, scoreResult.overallScore, now);
    for (const check of integrityResult.checks) {
        await candidatesQuery.insertIntegrityCheck((0, uuid_1.v4)(), candidateId, check.checkType, Boolean(check.flagged), check.title, check.explanation, check.evidence, check.confidence, now);
    }
    const scoreId = (0, uuid_1.v4)();
    await candidatesQuery.insertCandidateScore(scoreId, candidateId, scoreResult.overallScore, scoreResult.summary, JSON.stringify(scoreResult.requirements), now);
    return {
        candidateId,
...
const listCandidatesByRole = async (roleId, queryParams) => {
...
exports.listCandidatesByRole = listCandidatesByRole;
...
const compareCandidates = async (candidateIdA, candidateIdB) => {
    ...
    const matchedMustHaveA = reqsA.filter((r) => r.isMustHave && r.matched).length;
    const matchedMustHaveB = reqsB.filter((r) => r.isMustHave && r.matched).length;
    let explanation = '';
